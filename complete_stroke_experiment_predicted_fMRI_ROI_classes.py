...
from NIPS_utils import get_hemisphere_indices, get_roi_class_indices
...
num_iter_stroke = 2001 # 3201 max iterations
...
ROI_classes = ['floc-bodies', 'floc-faces', 'floc-places', 'floc-words']
# ROI_classes = ['floc-bo1dies', 'floc-faces']

# Creating image indices

# images_indices = np.arange(20)
# images_indices = [0]
...
root_save_path = '/home/jonathak/VisualEncoder/DIP_decoder/stroke_experiment_results_predicted_fMRI_ROI_classes'
...
for img_idx in images_indices:
    ...
    for ROI_class in ROI_classes:
        ...
        stroke_indices = get_roi_class_indices(stroke_sub, ROI_class)
        ...
        def closure():
            ...
            # Saving intermediate image 
            if save_throughout and i % save_every == 0 and i != 0:
                save_as_png(out_avg_np, f'{save_path}/img_{img_idx}_stroke_{ROI_class}_iter_{i}_start_original.png')
            i += 1
            return loss
        ...
        save_as_png(out_avg_np, f'{save_path}/img_{img_idx}_stroke_{ROI_class}_start_original.png')
        ...
        def closure():
            ...
            # Saving intermediate image 
            if save_throughout and i % save_every == 0 and i != 0:
                save_as_png(out_avg_np, f'{save_path}/img_{img_idx}_stroke_{ROI_class}_iter_{i}_start_fMRI.png')
        ...
        print(f'Finished decoding stroke fMRI for image {img_idx}, {ROI_class}')
    
        torch.cuda.empty_cache()
        # Saving the stroke fMRI final image
        save_as_png(out_avg_np, f'{save_path}/img_{img_idx}_stroke_{ROI_class}_start_fMRI.png')    
